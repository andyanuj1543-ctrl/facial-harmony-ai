...
type RenderMode = 'wireframe' | 'heatmap' | 'contours';
...
export const FaceMesh3DViewer: React.FC<FaceMesh3DViewerProps> = ({
  ...
  const [rotX, setRotX] = useState<number>(0.05); // slight tilt down
  const [rotY, setRotY] = useState<number>(0);    // yaw
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 2. Render Wireframe Edges / Lattice
      if (renderMode === 'wireframe' || renderMode === 'heatmap') {
        ctx.save();
        ctx.lineWidth = 1;
        ...
        if (showDenseLattice) {
          for (let i = 0; i < meshEdges.length; i++) {
            ...
            if (renderMode === 'heatmap') {
              ...
              // Cyber Gold with subtle depth fading
      ...
      // 3. Render Major Anatomical Contours
      ctx.save();
      ctx.lineWidth = renderMode === 'contours' ? 2.5 : 1.8;
      ctx.lineJoin = 'round';
      ctx.lineCap = 'round';
      ...
      Object.entries(FACIAL_CONTOURS_3D).forEach(([name, loop]) => {
        ...
        else if (name === 'midline') strokeColor = '#f43f5e88';
  ...
  return (
    <div className="flex flex-col w-full bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
      {/* Header Bar */}
      <div className="p-4 px-6 border-b border-slate-800 flex flex-wrap items-center justify-between gap-4 bg-slate-950/60">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Interactive Euclidean depth coordinates • {gender.toUpperCase()} Anthropometry
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-xl bg-slate-900 border border-slate-800">
          ...
          <button
            onClick={() => setRenderMode('contours')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              renderMode === 'contours'
                ? 'bg-amber-500 text-slate-950 shadow-md font-bold'
                : 'text-slate-400 hover:text-white'
      ...
      <div 
        ref={containerRef}
        className="relative w-full aspect-[4/3] md:aspect-[16/10] min-h-[440px] bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-hidden select-none cursor-grab active:cursor-grabbing"
      ...
      >
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-none">
          ...
          <div className="flex items-center gap-1.5 pointer-events-auto bg-slate-950/80 backdrop-blur-md p-1 rounded-xl border border-slate-800">
            ...
            <button
              onClick={() => snapAngle(75, 0)}
              className="px-2.5 py-1 rounded-lg text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
            >
              Profile (75°)
            </button>
            <button
              onClick={() => snapAngle(0, 50)}
              className="px-2.5 py-1 rounded-lg text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
            >
              Top View
            </button>
          ...
          <div className="flex items-center gap-2 pointer-events-auto">
            <button
              onClick={() => setShowDenseLattice(!showDenseLattice)}
              title="Toggle Dense Mesh Triangulation"
            ...
            >
              ...
              <span>{autoRotate ? 'Rotating' : 'Auto 360°'}</span>
            </button>
