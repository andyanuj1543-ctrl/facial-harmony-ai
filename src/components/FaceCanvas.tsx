...
export const FaceCanvas: React.FC<FaceCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      // Set canvas display dimensions to match natural aspect ratio
      const maxWidth = 720;
      ...
      // If no valid landmarks or no face detected, do NOT draw any face lines!
      if (!landmarks || landmarks.length < 468 || !metrics) return;

      // Scale landmarks to current canvas size
      ...
      if (overlayOptions.showThirds && metrics) {
        ctx.save();
        const thirdLines = [
      ...
      // 2. RULE OF FIFTHS OVERLAY
      if (overlayOptions.showFifths && metrics) {
        ctx.save();
      ...
      if (overlayOptions.showMidline && metrics) {
        ctx.save();
        ...
        ctx.fillRect(midX - 55, menton.y + 25, 110, 22);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 11px Plus Jakarta Sans, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(`Symmetry: ${metrics.symmetryScore}%`, midX, menton.y + 40);
        ctx.restore();
      }

      // 4. CANTHAL TILT VECTORS
      if (overlayOptions.showTilt && metrics) {
        ctx.save();
        ...
        ctx.fillRect(lOuter.x + 10, lOuter.y - 12, 100, 22);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 10px Plus Jakarta Sans, sans-serif';
        ctx.textAlign = 'left';
  ...
  return (
    <div className="relative w-full flex items-center justify-center bg-slate-900/60 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl p-2 min-h-[380px]">
      ...
      {/* Warning Overlay when NO face is found */}
      {detectionError && (
